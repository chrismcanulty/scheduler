...
describe("Appointment", () => {
  ...
  it("loads data, books an interview and reduces the spots remaining for Monday by 1", async () => {
    ...
    expect(getAllByTestId(container, 'spots-remaining')[0]).toHaveTextContent(/no spots remaining/i)
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Enter a new student name and select a new interviewer
    ...
    // 7. Wait until the appointment with the text "Lydia Miller-Jones" is displayed.

    await waitForElement(() => getByText(appointment, "Lydia Miller-Jones"));

    // 8. Check that spots remaining is unchanged

    const day = getAllByTestId(container, "day").find(day =>
      queryByText(day, "Monday")
    );

    expect(getAllByTestId(container, 'spots-remaining')[0]).toHaveTextContent(/1 spot remaining/i)
